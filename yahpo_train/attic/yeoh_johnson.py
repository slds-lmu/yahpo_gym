class ContNormalization(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.normalize == "scale":
            ...
            x = (x - self.min) / (
                (self.max - self.min) / (1.0 - 2 * self.sigmoid_p)
            ) + self.sigmoid_p
        return x.float()
    ...
    def trafo_yj(self, x, lmbda):
        return torch.where(
            x >= torch.as_tensor(0, dtype=torch.float64),
            self.scale_pos(x, lmbda),
            self.scale_neg(x, lmbda),
        )
    ...
    def scale_pos(self, x, lmbda):
        if torch.abs(lmbda).numpy() < self.eps:
            ...
            x = (_float_power(x + 1.0, lmbda) - 1) / lmbda
        return x

    def scale_neg(self, x, lmbda):
        if torch.abs(lmbda - 2.0).numpy() <= self.eps:
            x = torch.log1p(-x)
        else:
            x = (_float_power(-x + 1, 2.0 - lmbda) - 1.0) / (2.0 - lmbda)
        return -x
    ...
    def _neg_loglik(self, lmbda, x_sample):
        ...
        loglik += (lmbda - 1.0) * torch.sum(
            torch.sign(x_sample) * torch.log1p(torch.abs(x_sample))
        )
        return -loglik
    ...
    def inverse_trafo_yj(self, x, lmbda):
        ...
        return torch.where(
            x >= torch.as_tensor(0, dtype=torch.float64),
            self.inv_pos(x, lmbda),
            self.inv_neg(x, lmbda),
        )
    ...
    def inv_pos(self, x, lmbda):
        if torch.abs(lmbda) <= self.eps:
            ...
            x = _float_power(x * lmbda + 1.0, 1.0 / lmbda) - 1.0
        return x

    def inv_neg(self, x, lmbda):
        if torch.abs(lmbda - 2.0) < self.eps:
            # x = -torch.exp(x)+1.
            x = -torch.expm1(-x)
        else:
            x = 1.0 - _float_power(-(2.0 - lmbda) * x + 1.0, 1.0 / (2.0 - lmbda))
        return x
    ...
    def _clip_outliers(self, x_sample):
        ...
        """
        q = 0.995
        q1, q0 = torch.quantile(x_sample, q), torch.quantile(x_sample, 1.0 - q)
        iqr = torch.abs(q1 - q0)
