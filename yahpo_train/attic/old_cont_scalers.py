...
class ContTransformerNegExp(nn.Module):
    ...
    def __init__(self, x, scale=True, **kwargs):
        super().__init__()
        self.max = torch.as_tensor(1.0).to(torch.double)
        if scale:
...
class ContTransformerLog(nn.Module):
    ...
    def __init__(self, x, logfun=torch.log, expfun=torch.exp, eps=1e-12, **kwargs):
        self.logfun = logfun
        self.expfun = expfun
        self.eps = eps
        super().__init__()
...
class ContTransformerMultScalar(nn.Module):
    ...
    def __init__(self, x, m=1.0, **kwargs):
        super().__init__()
...
class ContTransformerFun(nn.Module):
    ...
    def __init__(self, x, trafo=lambda x: x, inverse=lambda x: x, **kwargs):
        super().__init__()
        self.trafo = trafo
...
class ContTransformerClipOutliers(nn.Module):
    ...
    def __init__(self, x, q=0.99, **kwargs):
        super().__init__()
        self.q = q
        self.q1, self.q0 = torch.quantile(x[~torch.isnan(x)], q), torch.quantile(
            x[~torch.isnan(x)], 1.0 - q
        )

    def forward(self, x, **kwargs):
        """
        Batch-wise transform for x. Clip values that are larger/smaller than a quantile + IQR (range between quantiles).
        """
...
def float_pow10(base):
    return _float_power(base, torch.as_tensor(10.0))
