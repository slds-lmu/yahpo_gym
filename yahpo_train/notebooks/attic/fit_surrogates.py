...
def fit_config(
    ...
    if dls_train is None:
        dls_train = dl_from_config(
            ...
            bs=bs,
            frac=frac,
            save_df_test=save_df_test_encoding,
            save_encoding=save_df_test_encoding,
        )  # train_frac is set to 0.8, and valid frac within train frac to 0.2, so for frac = 1 we have 0.6 train, 0.2 valid and 0.2 test

    # Construct embds from transforms
    # tfms overwrites emdbs_dbl, embds_tgt
    if tfms is not None:
        embds_dbl = [
            tfms.get(name) if tfms.get(name) is not None else ContTransformerRange
            for name, cont in dls_train.all_cols[dls_train.cont_names].iteritems()
        ]
        embds_tgt = [
            tfms.get(name) if tfms.get(name) is not None else ContTransformerRange
            for name, cont in dls_train.ys.iteritems()
        ]
    ...
    l = SurrogateTabularLearner(
        dls_train, f, loss_func=nn.MSELoss(reduction="mean"), metrics=nn.MSELoss
    )
    l.metrics = [
        AvgTfedMetric(mae),
        AvgTfedMetric(r2),
...
def tune_config(key, name, tfms_fixed={}, **kwargs):
    ...
    cc = cfg(key)
    dls_train = dl_from_config(
        cc, bs=10240, frac=1.0
    )  # set data loader up once, also save dls_train and encoding
...
def fit_from_best_params(key, best_params, tfms_fixed={}, log_wandb=False, **kwargs):
    ...
    if best_params.get("opt_deep_arch"):
        deep = get_arch(
            best_params.get("deep_u"),
            best_params.get("deep_n"),
            best_params.get("deep_s"),
        )
        use_deeper = best_params.get("use_deeper")
        if use_deeper:
            deeper = get_arch(
                best_params.get("deeper_u"),
                best_params.get("deep_n") + 2,
                best_params.get("deep_s"),
            )
        else:
            deeper = []
...
def fit_rbv2_super(key="rbv2_super", **kwargs):
    ...
    [
        tfms.update({k: tfms_chain([ContTransformerLog, ContTransformerRange])})
        for k in ["svm.cost", "svm.gamma"]
    ]
    [
        tfms.update({k: ContTransformerClamp0LogRange})
        for k in ["timetrain", "timepredict"]
    ]
    [
        tfms.update(
            {
                k: tfms_chain(
                    [
                        partial(
                            ContTransformerLog, logfun=torch.log2, expfun=torch.exp2
                        ),
    ...
    [
        tfms.update(
            {k: tfms_chain([partial(ContTransformerNegExp), ContTransformerRange])}
        )
        for k in ["logloss"]
    ]
...
def fit_rbv2_xgboost(key="rbv2_xgboost", **kwargs):
    ...
    [
        ...
        for k in ["nrounds", "eta", "gamma", "lambda", "alpha", "min_child_weight"]
    ]
    [
        tfms.update(
            {k: tfms_chain([partial(ContTransformerNegExp), ContTransformerRange])}
...
def fit_rbv2_ranger(key="rbv2_ranger", **kwargs):
    ...
    [
        tfms.update({k: ContTransformerLog2Range})
        for k in ["num.trees", "min.node.size", "num.random.splits"]
    ]
    [
        tfms.update(
            {k: tfms_chain([partial(ContTransformerNegExp), ContTransformerRange])}
...
def fit_rbv2_rpart(key="rbv2_rpart", **kwargs):
    ...
    [
        tfms.update({k: ContTransformerRange})
        for k in ["maxdepth", "minsplit", "minbucket"]
    ]
    [
        tfms.update({k: ContTransformerClamp0LogRange})
        for k in ["timetrain", "timepredict"]
...
def fit_fcnet(key="fcnet", **kwargs):
    ...
    [
        tfms.update({k: ContTransformerRange})
        for k in ["batch_size", "n_units_1", "n_units_2"]
    ]
    [
        tfms.update({k: ContTransformerLog2Range})
        for k in ["init_lr", "runtime", "n_params"]
    ]
    [
        tfms.update(
            {k: tfms_chain([partial(ContTransformerNegExp), ContTransformerRange])}
...
def fit_lcbench(key="lcbench", **kwargs):
    ...
    [
        ...
        for k in ["val_accuracy", "val_balanced_accuracy", "test_balanced_accuracy"]
    ]
    ...
    [
        ...
        for k in ["val_cross_entropy", "test_cross_entropy"]
    ]
    fit_config(key, tfms=tfms, **kwargs)
...
def fit_taskset(key="taskset", **kwargs):
        ...
        tfms.update(
            {
                k: tfms_chain(
                    [
                        partial(ContTransformerLog, logfun=torch.log, expfun=torch.exp),
                        ContTransformerRange,
                    ]
                )
            }
    ...
    [
        ...
        for k in ["train", "valid1", "valid2", "test"]
    ]
    fit_config(key, tfms=tfms, **kwargs)
...
def fit_iaml_ranger(key="iaml_ranger", **kwargs):
    ...
    [
        tfms.update({k: ContTransformerLogRange})
        for k in [
            "timetrain",
            "timepredict",
            "ramtrain",
    ...
    [
        tfms.update({k: partial(ContTransformerNegExpRange, q=0.975)})
        for k in ["logloss"]
    ]
...
def fit_iaml_rpart(key="iaml_rpart", **kwargs):
    ...
    [
        tfms.update({k: ContTransformerLogRange})
        for k in ["timetrain", "timepredict", "ramtrain", "rammodel", "rampredict"]
    ]
    [tfms.update({k: ContTransformerLog2Range}) for k in ["cp"]]
...
def fit_iaml_glmnet(key="iaml_glmnet", **kwargs):
    ...
    tfms = {}
    [
        tfms.update({k: tfms_chain([ContTransformerInt, ContTransformerRange])})
        for k in ["nf"]
    ]
    [
        tfms.update({k: partial(ContTransformerRange)})
        for k in ["auc", "ias", "mec", "mmce", "rammodel", "ramtrain", "timepredict"]
    ]
    [
        tfms.update({k: partial(ContTransformerLogRange)})
        for k in ["alpha", "rampredict", "timetrain", "logloss", "s", "trainsize"]
    ]
...
def fit_iaml_xgboost(key="iaml_xgboost", **kwargs):
    ...
    [
        tfms.update({k: ContTransformerRange})
        for k in ["mmce", "f1", "auc", "mec", "ias", "nf", "max_depth"]
    ]
    [
        tfms.update({k: partial(ContTransformerLogRange)})
        for k in ["timetrain", "timepredict", "ramtrain", "rammodel", "rampredict"]
...
if __name__ == "__main__":
    ...
    study_xgboost = tune_config(
        "iaml_xgboost", name="tune_iaml_xgboost_new", tfms_fixed=tfms_xgboost
    )
    ...
    l = fit_from_best_params(
        "iaml_xgboost",
        best_params=study_xgboost.best_params,
        tfms_fixed=tfms_xgboost,
        export=True,
        device=device,
        epochs=100,
    )
    ...
    fit_from_best_params(
        "iaml_super",
        study_super.best_params,
        tfms_fixed=tfms_super,
        log_wandb=True,
        export=True,
        epochs=100,
    )
