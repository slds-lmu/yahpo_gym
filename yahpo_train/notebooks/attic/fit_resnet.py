if __name__ == "__main__":
    ...
    for key in keys:
        ...
        if not cuda_available:
            raise ValueError(
                "No cuda device available. You probably do not want to fit on CPUs."
            )

        storage_name = "sqlite:///{}.db".format(study_path + "tune_" + key + "_resnet")
        study = optuna.load_study(
            study_name="tune_" + key + "_resnet", storage=storage_name
        )
        ...
        dls_train = dl_from_config(
            cfg(key), bs=bs, save_df_test=True, save_encoding=True
        )

        # tfms see tfms_list above
        l = fit_from_best_params_resnet(
            key,
            best_params=best_params,
            tfms_fixed=tfms_list.get(key),
            export=False,
            device="cuda:0",
            dls_train=dls_train,
        )
        ...
        l_noisy = fit_from_best_params_resnet(
            key,
            best_params=best_params,
            tfms_fixed=tfms_list.get(key),
            noisy=True,
