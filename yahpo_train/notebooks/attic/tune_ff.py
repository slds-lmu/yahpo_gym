...
if __name__ == "__main__":
    ...
    [
        tfms_iaml_super.update({k: ContTransformerLogRange})
        for k in ["mmce", "ramtrain", "rampredict", "timepredict", "ias"]
    ]
    [tfms_iaml_super.update({k: ContTransformerRange}) for k in ["f1", "auc"]]
    [
        tfms_iaml_super.update({k: ContTransformerNegExpRange})
        for k in ["logloss", "rammodel", "timetrain", "mec"]
    ]
    [
        tfms_iaml_super.update(
            {k: tfms_chain([ContTransformerInt, ContTransformerRange])}
        )
        for k in ["nf"]
    ]
    ...
    [
        tfms_iaml_xgboost.update({k: ContTransformerLogRange})
        for k in ["auc", "logloss", "ramtrain", "rammodel", "timepredict", "ias"]
    ]
    [tfms_iaml_xgboost.update({k: ContTransformerRange}) for k in ["f1"]]
    [
        tfms_iaml_xgboost.update({k: ContTransformerNegExpRange})
        for k in ["mmce", "rampredict", "timetrain", "mec"]
    ]
    [
    ...
    [
        tfms_iaml_ranger.update({k: ContTransformerLogRange})
        for k in ["mmce", "ramtrain", "rammodel"]
    ]
    [
        tfms_iaml_ranger.update({k: ContTransformerRange})
        for k in ["f1", "auc", "rampredict", "mec"]
    ]
    [
        tfms_iaml_ranger.update({k: ContTransformerNegExpRange})
        for k in ["logloss", "timetrain", "timepredict", "ias"]
    ]
    [
        tfms_iaml_ranger.update(
            {k: tfms_chain([ContTransformerInt, ContTransformerRange])}
        )
        for k in ["nf"]
    ]
    ...
    [
        tfms_iaml_rpart.update({k: ContTransformerLogRange})
        for k in ["mmce", "ramtrain", "rammodel", "timetrain", "timepredict", "ias"]
    ]
    [
        tfms_iaml_rpart.update({k: ContTransformerRange})
        for k in ["f1", "auc", "rampredict"]
    ]
    [
        tfms_iaml_rpart.update({k: ContTransformerNegExpRange})
        for k in ["logloss", "mec"]
    ]
    [
    ...
    [
        tfms_iaml_glmnet.update({k: ContTransformerLogRange})
        for k in [
            "mmce",
            "logloss",
            "ramtrain",
    ...
    parser.add_argument(
        "--name",
        type=str,
        default="tune_iaml_glmnet_ff",
        help='Name of the optuna study, e.g., "tune_iaml_glmnet_ff"',
    )
    parser.add_argument(
        "--trials", type=int, default=0, help="Number of optuna trials"
    )  # by default we run until terminated externally
    parser.add_argument(
        "--walltime", type=int, default=0, help="Walltime for optuna timeout in seconds"
    )  # by default we run until terminated externally
