...
def objective(trial, bench, opt_space):
...
def objective_mf(trial, bench, opt_space, fidelity_param_id, valid_budgets):
    ...
    for budget in valid_budgets:
        ...
        y = bench.objective_function(X)
        trial.report(- float(y.get("val_accuracy")), step=budget)  # FIXME: should be changed, see #21, #20
...
valid_budgets = precompute_budgets(max_budget, reduction_factor, sh_iters)
study.optimize(
    func=partial(
        objective_mf,
        bench=bench,
